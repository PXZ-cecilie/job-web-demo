...
'''
# datatable records the user & article relations
class UserArticle(Base):
    ...
    user_id = db.Column(db.BigInteger, db.ForeignKey('user.id', ondelete='SET NULL'))
    user = db.relationship('User', uselist=False, backref=db.backref('user_article', lazy='dynamic'))
    status = db.Column(db.SmallInteger, default=STATUS_NOT_FAV, index=True)

    def is_fav(self):
        self.status = self.STATUS_FAV

    def not_fav(self):
        self.status = self.STATUS_NOT_FAV
'''
